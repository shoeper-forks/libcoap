...
#define RS_DEFAULT_PORT COAPS_DEFAULT_PORT
...
main(int argc, char **argv) {
  ...
  if (app) {
    ...
    listen_addr.addr.sin6.sin6_port = htons(RS_DEFAULT_PORT);
...
init_resource(coap_context_t *ctx) {
   ...
   * omit leading '/') */
  resource = coap_resource_init((unsigned char *)RESOURCE, 
				strlen(RESOURCE), 0);
  if (resource) {
