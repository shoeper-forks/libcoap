...
static inline int
is_secure(const coap_endpoint_t *interface) {
  return interface && (interface->flags & COAP_ENDPOINT_DTLS);
}

ssize_t
send_to_peer(struct coap_context_t *coap_context,
	     const coap_endpoint_t *local_interface,
	     const coap_address_t *remote, 
	     unsigned char *data, size_t len) {
...
static void
handle_read(coap_application_t *app, coap_endpoint_t *local) {
...
#define INET6_ADDRSTRLEN 40
    ...
    if (is_secure(local)) {
      ...
      dtls_handle_message(app->dtls_context, &session,
			  (uint8 *)buf, bytes_read);
    } else {
      coap_handle_message(app->coap_context, local, &remote,
...
void
coap_free_application(coap_application_t *app) {
  coap_endpoint_t *ep;
...
static struct list_ep_t *
find_ep_item(coap_application_t *application, int handle) {
  struct list_ep_t *ep_item;
...
coap_endpoint_t *
coap_application_find_endpoint(coap_application_t *application,
			       int handle) {
  struct list_ep_t *ep_item;
...
coap_application_push_data_item(coap_application_t *application,
  ...
  if (ep_item) {
    ...
    data_item = (struct network_data_item *)
      coap_malloc(sizeof(struct network_data_item) + data_len);
...
int
coap_application_attach(coap_application_t *application,
...
void
coap_application_detach(coap_application_t *application,
			coap_endpoint_t *endpoint) {
  struct list_ep_t *ep_item;
...
static void
coap_check_retransmit(coap_context_t *context, coap_tick_t *next) {
  coap_tick_t now;
...
coap_err_t
coap_application_run(coap_application_t *application) {
  fd_set readfds;
...
ssize_t
coap_application_sendmsg(coap_application_t *application,
			 coap_endpoint_t *local_interface,
			 coap_address_t *dst, coap_pdu_t *pdu, int flags) {
  struct network_data_item *data_item;
  struct list_ep_t *ep_item = 
...
#define INET6_ADDRSTRLEN 40
	...
	if (coap_print_addr(dst, addr, INET6_ADDRSTRLEN+8)) {
      ...
      bytes_written = send_to_peer(application->coap_context,
				   local_interface, dst,
  ...
  if (pdu->hdr->type == COAP_MESSAGE_CON) {
    if (coap_send_confirmed(application->coap_context, local_interface, 
			    dst, pdu) == COAP_INVALID_TID) {
      debug("coap_application_sendmsg: cannot send confirmable message\n");
      coap_delete_pdu(pdu);
    }
  } else {
    if (coap_send(application->coap_context, local_interface, 
		  dst, pdu) == COAP_INVALID_TID) {
